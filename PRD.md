...
**Crash rule:** a job must never be stuck in `processing` forever. If the worker running it dies (including `SIGKILL` — no cleanup handler will run), the system must detect this and recover the job so it can run again. With your default settings, worst-case recovery must be under 60 seconds — our automated tests enforce this.
...
Failed jobs retry automatically after a delay of:

```text
delay = base ^ attempts seconds
```

where `attempts` is the number of completed attempts (so with `base = 2`: first retry after 2s, then 4s, 8s).

Default base is `2`, configurable via:

```text
config set backoff-base
```
...
re-enqueues a dead job. Decide whether it resets `attempts` and justify the choice in `DECISIONS.md`.
